 ...
 */
  ...
  new DifferentialDrive(m_leftMotorLeader::set, m_rightMotorLeader::set);
  ...
  //  String auton = SmartDashboard.getString("Auto Selector", "middle");
    //switch(auton) {
      ...
      // case "middle":
        ...
        if(time1.get()>6 && time1.get() <6.5){
          m_fwBottom.set(.6);
          m_fwTop.set(.6);
        }
        if(time1.get()>6.5 && time1.get() <7){
          m_itBottom.set(.5);
          m_itTop.set(.5);
        }
        ...
        //if(time1.get()>7.5 && time1.get() <8.3){
          //m_leftMotorLeader.set(.5);
          //m_rightMotorLeader.set(.5);
        //}
        if(time1.get()>7.5){
  ...
  }else if (m_stick.getRawButton(PS4Controller.Button.kR1.value)){
